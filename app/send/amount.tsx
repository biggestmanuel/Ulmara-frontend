...
export default function SendAmount() {
  ...
  const params = useLocalSearchParams<{ accountId: string; recipientName: string; asset: string; wallets?: string }>();
  const [amount, setAmount] = useState('');
  const [error, setError] = useState('');
  const continueFlow = () => {
    const value = parseFloat(amount);
    if (!value || value <= 0) return setError('Enter a valid amount');
    router.push({ pathname: '/send/network-select', params: { ...params, amount } });
  };
